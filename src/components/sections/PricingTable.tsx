...
export function PricingTable() {
  ...
  return (
    <section className="w-full py-16 px-6 lg:py-24 bg-surface-card border-t border-gray-100/50">
      <div className="max-w-7xl mx-auto">
            ...
            return (
              ...
              >
                ...
                <div className="text-center mb-6 pt-4 border-b border-gray-100 pb-8">
                  <h3 className="text-2xl mt-2 font-extrabold text-text-heading mb-3">
                    {card.planName}
                  </h3>
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    {card.pricing.annualOption && (
                      <p className="text-xs font-bold text-accent-saffron mt-3 bg-accent-saffron/10 px-2.5 py-1 rounded">
                        {card.pricing.annualOption}
                      </p>
                    )}
                  </div>
                ...
                <div className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-lg p-3 mx-2 text-center mb-6 shadow-sm">
                  <p className="text-xs font-bold text-[#166534] tracking-wide">
                ...
                <div className="flex-grow mb-8 px-2">
                  <ul className="space-y-4">
                    {card.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3.5 text-sm md:text-base text-text-body font-semibold">
                        <CheckSVG />
                ...
                <div className="mt-auto px-2 pt-6 pb-2 border-t border-gray-100">
                  <WhatsAppButton 
                    href={`https://wa.me/?text=${encodeURIComponent(card.ctaButton.preFilledWhatsAppMessage)}`}
                    className={`w-full py-4 shadow-sm text-sm ${!isPremium && 'bg-surface-base text-brand-dark border-2 border-brand-primary hover:bg-brand-primary/5 shadow-none'}`}
                  >
        ...
        {/* Comparison Anchor Strip Mapping */}
        <div className="w-full max-w-4xl mx-auto bg-surface-base border border-gray-200/60 rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between text-center md:text-left gap-6 mb-24 shadow-sm">
          <p className="text-text-body font-bold text-sm md:text-base max-w-md lg:max-w-xl leading-relaxed">
            {data.comparisonAnchorStrip.stripCopy}
          </p>
          <SecondaryLink 
            href={`https://wa.me/?text=${encodeURIComponent(data.comparisonAnchorStrip.preFilledWhatsAppMessage)}`}
            className="flex-shrink-0"
          >
            {data.comparisonAnchorStrip.ctaLabel}
          </SecondaryLink>
        </div>

        {/* Guarantee Block Component mapped cleanly strictly bypassing toggle dependencies */}
        <div className="w-full rounded-2xl bg-accent-guarantee p-8 md:p-12 xl:p-16 mb-12 border border-accent-guarantee/60 flex flex-col items-center text-center shadow-inner">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-card-base mb-6 text-brand-primary">
            <ShieldSVG />
          </div>
          <span className="text-brand-primary font-bold uppercase tracking-widest text-xs md:text-sm mb-4">
            {data.guaranteeBlock.eyebrowLabel}
          </span>
          <h3 className="text-text-heading text-2xl md:text-4xl font-extrabold mb-6 leading-tight max-w-3xl">
            {data.guaranteeBlock.headline}
          </h3>
          <p className="text-text-body text-base md:text-lg mb-8 leading-relaxed max-w-4xl font-medium">
            {data.guaranteeBlock.body}
          </p>
          <p className="text-brand-dark text-sm md:text-base font-extrabold italic border-t border-brand-primary/20 pt-6 max-w-3xl">
            {data.guaranteeBlock.supportingLine}
          </p>
          
          <div className="mt-8 text-left max-w-4xl w-full bg-white/60 p-6 rounded-lg text-xs md:text-sm font-semibold text-text-body leading-relaxed border border-white/80 shadow-sm text-balance">
            {data.guaranteeBlock.expandCollapse.expandedTermsCopy}
          </div>
        </div>

        {/* No Joining Fee Strip */}
        <div className="text-center py-5 px-4 bg-brand-primary/10 rounded-xl mb-24 max-w-3xl mx-auto border border-brand-primary/20 hover:bg-brand-primary/15 transition-colors cursor-default">
          <p className="text-brand-primary font-extrabold text-xs md:text-sm tracking-widest uppercase flex items-center justify-center gap-2">
            <ClockSVG />
            {data.noJoiningFeeStrip.stripCopy}
          </p>
        </div>
